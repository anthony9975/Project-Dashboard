...
- **Roadmap steps have their own nested to-do lists.** Steps are drag-to-reorder; to-dos
  are not (order doesn't matter there). A step auto-expands its to-dos when set to "in
  progress," and auto-completes once every to-do is done — but never auto-reverts, since
  manually marking a step done early is allowed and shouldn't get silently undone later.
