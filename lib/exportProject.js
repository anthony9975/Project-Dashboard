...
// a plain Markdown export, and (deliberately, for now) no static-image fallback either. See
// "Technical diagram" in Project_Dashboard_Design.md for the open question of how best to
// add one later (a base64-embedded image keeps the export a single portable file; switching
// export to a .zip with an assets/ folder keeps files clean but changes the download UX).
// Until that's decided, the export just notes that a diagram exists and points back to the
// live page rather than silently omitting it.
function renderDiagram(diagram) {
  ...
  return `_Interactive diagram attached (${diagram.originalFilename}, uploaded ${formatDate(
    diagram.uploadedAt
  )}) — view it on the project page. Static image export isn't supported yet._`;
}
