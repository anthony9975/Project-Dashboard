...
function normalizeProject(project) {
  ...
  result = normalizeTechnicalSpecs(result);
  return result;
}
...
function normalizeTechnicalSpecs(project) {
  // New field — projects that predate it just get an empty array on read rather than a
  // migration, same as any brand-new field would. Each entry is {id, label, values}; values
  // is always an array (even for a single value) since a spec's whole point is supporting
  // more than one value per label — e.g. a "Languages" spec with several entries.
  if (!Array.isArray(project.technicalSpecs)) {
    return { ...project, technicalSpecs: [] };
  }
  let changed = false;
  const technicalSpecs = project.technicalSpecs.map((s) => {
  ...
  return changed ? { ...project, technicalSpecs } : project;
}
