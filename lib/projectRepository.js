...
function normalizeProject(project) {
  // Roadmap steps used to be plain strings; they're now {id, text, status, todos} objects
  // so status can be tracked per step and per sub-task. Upgrade old data transparently on
  // read rather than requiring a manual migration — it gets re-saved in the new shape the
  // next time anything on the project changes.
  if (!Array.isArray(project.roadmap)) return project;
  let changed = false;
  const roadmap = project.roadmap.map((s, i) => {
    if (typeof s === 'string') {
      changed = true;
      return { id: `legacy-${i}`, text: s, status: 'not_started', todos: [] };
    }
