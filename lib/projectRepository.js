...
function normalizeProject(project) {
  // Roadmap steps used to be plain strings; they're now {id, text, status} objects so
  // status can be tracked per step. Upgrade old data transparently on read rather than
  // requiring a manual migration — it gets re-saved in the new shape the next time
  // anything on the project changes.
  if (Array.isArray(project.roadmap) && project.roadmap.some((s) => typeof s === 'string')) {
    return {
      ...project,
      roadmap: project.roadmap.map((s, i) =>
        typeof s === 'string' ? { id: `legacy-${i}`, text: s, status: 'not_started' } : s
      ),
    };
  }
  return project;
